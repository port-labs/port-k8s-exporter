...
func TestJqComplexEntityMapping(t *testing.T) {
	...
	t.Run("Entity mapping with complex alternative operators", func(t *testing.T) {
		// This mimics a complex version query with multiple alternative operators
		versionQuery := ".metadata.labels[\"app.kubernetes.io/version\"] // .metadata.labels.version // .metadata.labels.chartVersion // .metadata.labels[\"helm.sh/chart\"] | select(. != null) | split(\"-\") | last // .spec.template.metadata.labels[\"app.kubernetes.io/version\"] // \"unknown\""
